...
이제 세션 저장소로 In-Memory Database를 사용하기로 하였습니다. 

In-Memory Database는 다양한 종류가 있기 때문에 어떤 종류의 데이터베이스를 사용할지를 정해야 합니다.
...
1. Failover(장애 극복 기능)
    ...
    **Redis**
    ...
    Redis는 Replication을 지원해 줍니다. 서버 하나가 장애가 발생하면 복제된 Slave 서버를 Master로 승격시켜 줍니다.
    

    **Memcached**

    ![image5](/Img/Session/Session5.png)

    Memcached는 Replication을 지원하지 않는 대신에 ‘Consistent Hashing’ 알고리즘을 통해 여러 대의 Memcached 서버에 데이터를 분산 저장하여 관리합니다.
    ...
    Memcached 방식의 경우 서버의 숫자를 늘리면 피해를 감소시킬 수 있습니다.

    예를들어, 3대 중 1대에 장애가 발생하면, 약 33%의 데이터를 손실하지만, 4대 중 1대에서 장애가 발생한다면 25%의 데이터만 손실됩니다. 그러므로 노드가 많을수록 서비스 장애에 대한 피해는 줄어듭니다.

<br>

2. 응답속도의 균일성에서 차이가 발생할 수 있습니다.
    
    **Redis**
    
    Redis는 메모리 할당을 하는 데 있어서 jemalloc 알고리즘을 사용하여 매번 malloc과 free를 통하여 메모리를 할당하기 때문에 이로인해 메모리 단편화 문제를 발생시키고, 단편화 문제로 인하여 응답속도가 느려질 수 있습니다. 그래서 대규모 트래픽이 발생하면 Memcached에 비해 응답속도의 안정성이 떨어질 수 있습니다.
    
    **Memcached**
    
    Memcached는 메모리 할당에 있어서 slab allocator를 사용하기 때문에 내부적으로 메모리를 할당을 다시 하지 않고 관리할 수 있습니다.
    
<br>

3. Redis는 싱글 스레드로 동작하고, Memcached는 멀티 스레드로 동작한다.
    
    **Redis**
    
    Redis는 데이터가 메모리에서 저장되고, 관리되기 때문에 CPU가 Redis에서 병목현상을 발생시키는 빈도가 적습니다. 평균적으로 Linux 환경에서 Redis는 파이프라인을 사용하면 초당 약 100만건의 요청을 전달 가능하다고 제시하였습니다.
    
    그럼에도 불구하고 CPU를 더 많이 사용하기 위해서는 Replication과 Sharding을 활용하여 Redis의 인스턴스를 증가시키고, 각 인스턴스로 데이터를 분산시켜서 사용하면 가능합니다.
    
    **Memcached**
    
    Memcached가 멀티스레드로 동작하기 때문에 훨씬 좋은 성능을 내는 것 처럼 보일 수 있습니다.
    
    그러나 실제로는 Memcached는 명령어를 파싱 하는 것까지는 멀티 스레드 형태로 수행되지만, 실제 데이터에 접근하는 연산들의 경우에는 Global cache lock으로 인하여 서버 전체의 캐시에 대해 잠금 상태가되기 때문에 멀티 스레드로 구성되어있음에도 데이터 연산에 있어서는 싱글 스레드처럼 동기화되어서 동작합니다.
    
<br>

4. 실제 성능 비교
    
    실제 Memcached와 Redis 성능 비교를 해보았을때  Memcached의 경우 Write 연산이, Redis의 경우 Read 연산이 더 좋은 성능을 보입니다.
    
    이를 보았을때, Redis가 세션을 저장하는 데 더 적합하다는 점을 알 수 있습니다. 세션 작업의 경우 쓰기 연산보다 읽기 연산 작업이 더 많기 때문에 Read 연산이 좋은 Redis를 사용하는 것이 더 효율적임을 알 수 있습니다.